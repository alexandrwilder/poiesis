...
// `log_ window` opens the app in its own clean window: no tab bar, no title bar, black,
...
func openWindow(root string, record bool) error {
	...
	// inside LOG_.app the vault path comes from the bundle, so the command carries no
	...
	if bin, err := exec.LookPath("alacritty"); err == nil {
		args := []string{"-o", "window.decorations=\"None\"", "-o", fmt.Sprintf("window.dimensions.columns=%d", windowCols),
			...
			"-o", "colors.primary.background=\"#000000\"", "-T", "LOG_", "-e"}
	...
	return fmt.Errorf("no terminal found that can open a clean window (Ghostty, Alacritty, kitty or WezTerm); run `log_` in your terminal instead")
}

// findGhostty returns the Ghostty app (macOS) or binary (elsewhere), or "".
func findGhostty() string {
	if runtime.GOOS == "darwin" {
		// the copy inside LOG_.app first: it carries this app's name
...
// macAppBinary is the program inside LOG_.app, or "" when it is not installed.
func macAppBinary() string {
	app := outerAppBundle()
	if app == "" {
		return ""
	}
	p := filepath.Join(app, "Contents", "MacOS", "log_")
	if _, err := os.Stat(p); err != nil {
		return ""
	}
	return p
...
// writeGhosttyConfig puts the command and the LOG_ look in the app's own folder.
...
# the title bar keeps the three buttons and the drag area; LOG_ tints it with the picture's top edge
...
# LOG_ updates as one piece: the window host must never ask about updates
auto-update = off
